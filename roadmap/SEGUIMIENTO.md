...
**Última actualización:** 2026-09-01 — T-14 (detector de problemas de lectura en voz alta) COMPLETADA por una sesión de nube. `scripts/deteccion.py` nuevo: `detectar_problemas_bloque`/`detectar_problemas_guion` avisan de lo que va a costar decir sobre cada `BloqueRespiracion` (T-11) ya trozado, sin modificarlo — cinco familias, una por requisito. (1) `sin_punto_respiracion`: bloque por encima de un umbral configurable de palabras sin ninguna puntuación intermedia. (2) `cacofonia`: «de» encadenados en una ventana de palabras, sílaba inicial repetida entre palabras adyacentes, rima involuntaria por sufijo compartido. (3) `trabalenguas`: grupo de consonantes seguidas dentro de una palabra, o varias palabras largas seguidas. (4) `anglicismo`: tabla `ANGLICISMOS_COMUNES` (`config.py`) con equivalente en español. (5) `estructura_dificil`: incisos acumulados, subordinadas encadenadas, doble negación, voz pasiva larga. Ninguna familia reescribe (alcance decidido por el dueño, §0.2) salvo la excepción literal del requisito 6: `sin_punto_respiracion` marca `admite_particion=True` y adjunta una `particion_sugerida` (dos mitades de texto, partiendo por el nexo subordinante más cercano al centro), porque es la única que afecta al troceo — aplicarla de verdad sigue siendo alcance de T-15. Cada aviso indica severidad y recomendación; la localización (escena/bloque) vive en `ResultadoDeteccionBloque.bloque`, mismo patrón que T-13. Todos los umbrales nuevos son campos de `Configuracion` con validación de entero positivo (regla "sin números mágicos", §0.2). Verificado sin falsos positivos en texto cotidiano y con avisos reales (no vacíos, no desbordados) sobre los tres guiones reales de `fixtures/reales/`. Suite: 198 pasan + 2 skipped (antes 179 pasan + 2 skipped; T-14 no tenía talón pendiente en `tests/test_logica_pendiente.py`, así que suma 19 tests nuevos en `tests/test_deteccion.py` sin sustituir ningún skip). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-15 (reescrituras marcadas y reversibles).
>
