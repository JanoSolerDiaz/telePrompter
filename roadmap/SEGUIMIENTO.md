...
**Última actualización:** 2026-09-01 — T-04 (CI local + workflow inactivo) COMPLETADA por una sesión de nube. `scripts/ci.py` es ahora el único sitio con las cuatro verificaciones del protocolo (tipos, estilo, tests, extremo a extremo): las ejecuta en orden y hasta el final aunque alguna falle, imprime un resumen etapa a etapa y agrega el resultado en un único código de salida. El hook de pre-commit (`scripts/hooks/pre-commit`, T-01) deja de duplicar los cuatro comandos y pasa a invocar `python scripts/ci.py`, propagando su código de salida. Se añadió `.github/workflows/ci.yml`: mismo pipeline en GitHub Actions, preparado pero **inactivo a propósito** (solo `workflow_dispatch`, sin trigger `push`/`pull_request`) — activarlo sobre `origin/develop` es decisión del dueño, no de esta tarea. Detalle en `DECISIONES_TECNICAS.md`. Suite: 35 pasan + 8 skipped (antes 29 pasan + 8 skipped; +6 tests netos: 5 nuevos en `tests/test_ci.py`, y en `tests/test_hooks.py` las 2 aserciones sobre el contenido literal del hook se sustituyeron por 3 sobre la delegación en `ci.py` y la propagación de su código de salida). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-05 (monitorización de errores local).
