...
**Última actualización:** 2026-09-01 — T-11 (troceo en bloques de respiración) COMPLETADA por una sesión de nube. `scripts/troceo.py` nuevo: parte cada bloque `locucion` ya clasificado (T-09) en fragmentos de `palabras_por_bloque_min`-`palabras_por_bloque_max` palabras cortando por prioridad (requisito 1: puntuación fuerte → débil → nexos → sintagma), sin cortar nunca dentro de una cifra, fecha o sigla puenteada (requisito 2) y fundiendo los tramos por debajo del mínimo con el vecino más afín (requisito 3), repartiendo la fusión en dos si se pasa del máximo en vez de dejarla entera. Probado contra los tres guiones reales durante el desarrollo (no solo al final): el primer diseño abandonaba un nivel de prioridad entero volcando *todo* el resto del párrafo al siguiente nivel en cuanto no encontraba un candidato cercano, y la fusión ciega de tramos cortos dejaba solo 88.6% de los bloques en rango (por debajo del 90% exigido). Corregidos ambos (ver `DECISIONES_TECNICAS.md`): sobre los tres guiones reales, 234/234 bloques de respiración (100%) caen en `[6, 12]` palabras, 0 cortes forzados, y la suite añade `tests/test_troceo.py` (14 tests: criterio de aceptación sobre fixtures reales + unidad sobre `trocear_texto` con casos sintéticos de fecha/número con unidad/corte forzado). Suite: 144 pasan + 4 skipped (antes 130 pasan + 5 skipped; el skip de T-11 en `tests/test_logica_pendiente.py` se sustituye por 14 tests reales en `tests/test_troceo.py`). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-12 (motor de tiempos).
