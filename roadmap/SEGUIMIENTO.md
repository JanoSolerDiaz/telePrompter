...
**Última actualización:** 2026-09-01 — T-13 (normalización a forma dicha) COMPLETADA por una sesión de nube. `scripts/normalizacion.py` nuevo: `normalizar_texto` detecta y propone la forma dicha de un texto sin modificarlo, en orden de prioridad estricto por familia de regla (diccionario del dueño primero, después moneda/porcentaje/unidad/rango/fracción/ordinal/cardinal/sigla/símbolo suelto, y por último las conjunciones), marcando cada tramo resuelto como ocupado para que ninguna familia posterior lo reinterprete. Cubre cardinales y años (`2026` → «dos mil veintiséis»), ordinales del 1º al 10º (`1ª` → «primera»), porcentajes (`15 %` → «quince por ciento»), monedas con céntimos (`1.500 €` → «mil quinientos euros»), unidades abreviadas, rangos, fracciones, símbolos sueltos (`+`, `>`), siglas (deletreo letra a letra por defecto) y las conjunciones «y»/«e», «o»/«u» con la excepción del diptongo `hie-`/`hia-`. Apócope (`un`/`veintiún`) y concordancia de género se aplican solo cuando hay un sustantivo detrás del número, por heurística de sufijo con una lista corta de excepciones (`día`, `mano`...) — deliberadamente no perfecta: el diccionario del dueño (`diccionario-locucion.json`, requisito 3) siempre gana y corrige cualquier fallo. Cada `Normalizacion` conserva `original` y `propuesta` a la vez (invariante (b)); `aplicar_normalizaciones`/`deshacer_normalizaciones` son inversas exactas, verificado con texto sintético y con los tres guiones reales trozados por T-11. `normalizar_guion` opera sobre `BloqueRespiracion` (T-11, no antes del troceo, para no proponer una expresión que un corte de respiración parta en dos) y produce un resultado por bloque, incluso sin normalizaciones (cobertura total). `config.py` gana `NOMBRE_ARCHIVO_DICCIONARIO_LOCUCION`, `SIMBOLOS_MONEDA` y `UNIDADES_ABREVIADAS` como diccionarios planos (no como campos de `Configuracion`: la "sobreescritura por el dueño" que pide §0.2 ya la da el diccionario por entrada, ver `DECISIONES_TECNICAS.md`). Suite: 179 pasan + 2 skipped (antes 151 pasan + 3 skipped; el skip de T-13 en `tests/test_logica_pendiente.py` se sustituye por 28 tests reales en `tests/test_normalizacion.py`, incluidos los cuatro casos literales del criterio de aceptación y la prueba de reversibilidad sobre los tres guiones reales). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-14 (detector de problemas de locución).
>
