...
- **`tarjetas.json`** — contrato de intercambio con la generación del `.pptx` (T-29, `references/contrato-tarjetas.md`), pero reutilizable por cualquier consumidor: trae `duracion_estimada_segundos` por escena en el mismo orden que el guion, con lo que la fase de montaje puede calcular el instante de inicio/fin de cada escena sumando duraciones (el `.srt` no lleva ninguna marca de escena en su propio texto). Desde R-13 trae también `duracion_real_segundos` (la misma duración de la toma buena que usa `guion-alineado.srt`, `null` si la escena no la tiene todavía): sumar esa duración cuando existe, la estimada si no, reconstruye el rango de cada escena coherente con `guion-alineado.srt` en vez de con `guion.srt` — fórmula completa en `references/contrato-montaje.md`.
