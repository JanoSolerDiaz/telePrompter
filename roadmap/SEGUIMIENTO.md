...
**Última actualización:** 2026-09-01 — T-16 (`guion-escenas.md`, el documento de revisión de una sola pasada) COMPLETADA por una sesión de nube. `scripts/documento_revision.py` nuevo: compone en un único `.md` los resultados que ya calculan T-08 a T-15 (parseo, tiempos, detección, reescrituras), sin recalcular nada por su cuenta. Cabecera con instrucciones de edición y resumen global (escenas, palabras, duración, ritmo, avisos, reescrituras pendientes); una sección por escena reproduciendo el mismo encabezado `## BLOQUE N — <título>` del guion de origen, con sus bloques de respiración numerados desde 1 y delimitados por un ancla `<!-- bloque escena=N indice=K -->` (misma idea que el ancla de reescritura de T-15) para que una edición a mano del texto siga siendo localizable por `extraer_texto_bloques`; junto a cada bloque, sus reescrituras marcadas (T-15) y sus avisos de locutabilidad (T-14) que no dieron ya lugar a una reescritura de partición, para no repetir el mismo problema dos veces. Al pie de cada escena, las indicaciones no recitables (`**EN PANTALLA**`, `**NOTA**`, bloques `revisar`) con su motivo. Marca de estado de la revisión completa (`extraer_estado_revision`, `PENDIENTE`/`VALIDADO`) con la misma tolerancia de lectura que la marca de decisión de T-15, para que una futura T-17 sepa cuándo el dueño da la pasada por terminada. `guardar_documento_revision` nunca sobrescribe sin copiar antes a `.bak-<marca_de_tiempo>` (invariante (d)). Suite: 243 pasan + 2 skipped (antes 224 + 2; 19 tests nuevos en `tests/test_documento_revision.py`, incluida la cobertura total de escenas y bloques sobre los tres guiones reales, criterio de aceptación literal). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-17 (revalidación: releer, respetar y recalcular), que consumirá `extraer_texto_bloques` y `extraer_estado_revision` tal cual.
>
