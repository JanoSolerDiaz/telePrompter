...
**Última actualización:** 2026-09-01 — T-10 (detección de convención de marcado y propuesta de convención explícita) COMPLETADA por una sesión de nube. `scripts/convencion.py` nuevo: no descubre la convención (T-08/T-09 ya la implementan) ni decide si adoptarla (el dueño ya lo hizo: contractual, con aviso, §6 pregunta 3 / §0.2), la documenta y cierra sus huecos. `generar_convencion_guiones`/`guardar_convencion_guiones` (requisito 4) escriben `convencion-guiones.md`, una página lista para pegar en la plantilla del dueño, generada desde `Configuracion` (patrón de encabezado, rótulos de locución/no locución, secciones auxiliares, metadatos de cabecera) para no mantener un segundo texto a mano. `detectar_desviaciones` (requisito 5) señala, sin bloquear nunca el proceso: escena sin rótulo `**LOCUCIÓN**`, rótulo desconocido (una línea con forma de rótulo que no es ninguno de los configurados) y sección auxiliar no reconocida; el subtítulo entrecomillado tras el título del guión (evidencia de T-08) se reconoce por posición como categoría auxiliar conocida, no como desviación cada vez — sin este reconocimiento, los tres guiones reales habrían salido "desviados" por algo ya documentado como esperado. `medir_consistencia_senales`/`proponer_convenciones` (requisitos 1-2) miden si una señal de inferencia de T-09 decide siempre el mismo tipo y proponen adoptarla como convención explícita con ejemplo antes/después y ahorro, excluyendo las señales ya contractuales; el histórico de guiones procesados entre sesiones queda como parámetro de quien llama (`list[ResultadoClasificacion]`), sin inventar un almacén de persistencia sin caso real que lo ejercite (T-16/T-17 son quienes coordinarán procesar más de un guion). Criterio de aceptación verificado sobre los tres guiones reales: cero desviaciones y cero propuestas nuevas (ya se clasifican enteros por la ruta rápida de rótulo, sin apoyarse en ninguna señal de inferencia de contenido) y un guion sintético con una escena sin rótulo se procesa igual y aparece señalado. `parser.py` gana la constante `MOTIVO_SECCION_NO_RECONOCIDA` (antes un literal repetido) para que `convencion.py` distinga ese motivo sin duplicar el texto. Suite: 130 pasan + 5 skipped (antes 120 pasan + 5 skipped; +10 tests en `tests/test_convencion.py` nuevo). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-11 (troceo en bloques de respiración).
