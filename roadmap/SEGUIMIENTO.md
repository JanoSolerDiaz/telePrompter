...
**Última actualización:** 2026-09-01 — T-09 (clasificador locución / no locución) COMPLETADA por una sesión de nube. `scripts/clasificador.py` nuevo: `clasificar_guion`/`clasificar_escena` separan, dentro de cada escena ya parseada (T-08), el texto recitable del que no. Ruta rápida (requisito 1, manda siempre que esté presente): rótulo de sección, configurable (`Configuracion.rotulo_locucion`/`.rotulos_no_locucion`, campos nuevos con los mismos valores por defecto que ya vivían en `config.py`). Dentro de una sección `**LOCUCIÓN**`, el texto en cita de bloque (`> `) es locución; el texto suelto fuera de cita se marca `revisar` (requisito 3 — caso real en los tres guiones: acotaciones de ritmo entre citas, un encargo de ejemplo sin comillar). Sin rótulo activo, inferencia de respaldo (requisito 2): la propia cita de bloque ya es señal suficiente (sin necesidad del rótulo — es la misma convención), más marca de tiempo, acotación entre paréntesis/corchetes, prefijo (`PANTALLA:`/`B-ROLL:`/`NOTA:`/`IMAGEN:`/`TÍTULO:`), mayúsculas, negrita/cursiva de línea completa, checklist, tabla, enlace suelto, encabezado interno y bloque de código; sin señal clara, `revisar`, nunca se decide en silencio (requisito 5). Cada bloque lleva tipo, motivo, señal y rango de líneas (requisito 4); `reconstruir(bloques)` reproduce el `.md` de origen entero sin pérdida (cobertura total, invariante (a) de §0.2, requisito 6) — preámbulo, secciones auxiliares y el interior completo de cada escena. `ResumenEscena` por escena cuenta palabras excluidas y por qué motivo (requisito 7). Criterio de aceptación verificado sobre los tres guiones reales: 100% correcto por ruta rápida (cero pérdida de `**LOCUCIÓN**`, cero fuga de `**EN PANTALLA**`/`**NOTA**` como locución) y, con los mismos guiones despojados de sus rótulos, ≥95% de precisión en palabras de locución por inferencia (la cita de bloque sola ya distingue con exactitud en los tres). Se detectó y corrigió en el camino una trampa real de Python: `"a\nb\n".splitlines()` pierde la línea en blanco final de un `contenido` ya construido con `"\n".join(...)` (T-08); el módulo usa `.split("\n")`, su inverso exacto, documentado en `DEVELOPERS.md` para que no se repita. Suite: 120 pasan + 5 skipped (antes 109 pasan + 7 skipped; +11 tests en `tests/test_clasificador.py` nuevo, -2 skips: los talones `test_clasificador_distingue_locucion_de_no_locucion` y `test_invariante_cobertura_total_del_guion` de `tests/test_logica_pendiente.py` quedan implementados como su propio criterio de aceptación). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-10 (detección de convención de marcado y propuesta de convención explícita).
