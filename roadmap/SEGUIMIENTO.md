...
**Última actualización:** 2026-09-01 — T-15 (reescrituras marcadas, aceptables y reversibles) COMPLETADA por una sesión de nube. `scripts/reescrituras.py` nuevo: une las propuestas de T-13 (`Normalizacion`) y la partición de T-14 (`Aviso.particion_sugerida`, única familia con `admite_particion=True`) en un tipo único, `Reescritura`, con `id` estable (hash de escena+posición+familia+original, no del contenido de la propuesta) para que revalidar reconozca "la misma" reescritura aunque la regla cambie de texto sugerido. Formato marcado (`formatear_reescritura`/`extraer_decisiones`): bloque `<!-- reescritura id=... -->` con original/propuesta/motivo a la vista y una única palabra de decisión (`PENDIENTE`/`ACEPTAR`/`RECHAZAR`) que el dueño sobrescribe a mano, leída con una búsqueda tolerante a mayúsculas y a la sintaxis Markdown de alrededor, no por posición de columna. `fusionar_con_estado`/`guardar_en_estado` persisten en `estado.reescrituras` (ya existía desde T-07, sin migración): una decisión ya tomada nunca se vuelve a proponer como pendiente, y ninguna reescritura registrada desaparece aunque el guion cambie y deje de generarla. `texto_con_reescrituras_aceptadas` aplica solo las aceptadas sobre el texto del bloque; `aplicar_particion_aceptada`/`aplicar_particiones_aceptadas` materializan de verdad una partición aceptada (lo que T-14 dejó pendiente en su requisito 6), sustituyendo un `BloqueRespiracion` por los dos resultantes de partirlo. `revertir_reescrituras` deshace en bloque (una escena o el guion completo) sin perder el original. Suite: 224 pasan + 2 skipped (antes 198 + 2; 26 tests nuevos en `tests/test_reescrituras.py`, incluido el ciclo completo del criterio de aceptación literal — proponer, aceptar una, rechazar otra, revalidar — y una pasada sobre los tres guiones reales). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-16 (`guion-escenas.md` de una sola pasada), que ya tiene el bloque marcado de T-15 listo para insertar dentro de cada escena.
>
