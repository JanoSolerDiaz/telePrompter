...
**Última actualización:** 2026-09-01 — T-12 (motor de tiempos) COMPLETADA por una sesión de nube. `scripts/tiempos.py` nuevo: `calcular_tiempos` es la única función que calcula tiempos (requisito 4), a partir de un `ResultadoParseo` (T-08) ya parseado. Ritmo (requisitos 1, 7, 8): ppm único deducido de las palabras de locución totales frente a la suma de las duraciones por escena (respaldo 120 ppm si falta alguna o el valor cae fuera de `[90, 180]`; calibración manual con prioridad sobre ambos), siempre transparente (`RitmoAplicado` con origen, valor deducido aunque se descarte, y el valor alternativo). Duración por bloque (requisito 2): palabras al ritmo aplicado más pausa por puntuación final (coma < punto < fin de párrafo < fin de escena, estas dos últimas por posición, no por puntuación). Agregados por escena y total sin descuadre de redondeo (requisitos 3-4, un único cursor acumulado). Contraste (requisito 6) por escena y en total, avisando cuántas palabras sobran o faltan. **Hallazgo real corregido durante el desarrollo, no al final** (ver `DECISIONES_TECNICAS.md`): un primer diseño trataba el rango horario de un encabezado de escena (`(1:55 – 3:10)`, dos marcas de tiempo del vídeo) igual que el metadato `**Duración objetivo:**` (`3:40 – 3:55`, una horquilla real de duración), promediando ambos extremos como si fueran una horquilla de duraciones posibles para la escena; eso deducía un ppm de ~33-47 en los tres guiones reales, muy fuera de la banda de plausibilidad. Corregido separando las dos semánticas (duración de escena = diferencia de los dos extremos; duración total = el metadato tal cual, con la suma de escenas como respaldo): sobre los tres guiones reales se deduce un ppm de 140-167 (dentro de `[90, 180]`), origen `deducido` en los tres. Suite: 151 pasan + 3 skipped (antes 144 pasan + 4 skipped; el skip de T-12 en `tests/test_logica_pendiente.py` se sustituye por 8 tests reales en `tests/test_tiempos.py`). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-13 (normalización a forma dicha).
