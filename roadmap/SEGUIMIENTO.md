...
**Última actualización:** 2026-09-01 — T-18 (esqueleto del reproductor autocontenido) COMPLETADA por una sesión de nube. `scripts/reproductor.py` nuevo: genera el artefacto principal de la skill, un unico `.html` con doble clic, offline. No calcula nada por su cuenta — toma un `ResultadoParseo` (T-08) y un `ResultadoTiempos` (T-12) ya calculados y los compone, mismo patron que `documento_revision` (T-16). Requisito 1 (plantillas en `assets/`): tres archivos nuevos en `assets/reproductor/` (`plantilla.html`, `estilo.css`, `guion.js`) unidos por sustitucion de marcadores, no f-strings de Python. Requisito 2 (fuentes del sistema, neutro y oscuro): colores y pila tipografica nuevos en `Configuracion` (`color_fondo_reproductor`, `color_texto_reproductor`, `color_texto_secundario_reproductor`, `pila_tipografica_reproductor`, `tamano_texto_base_px`), todo sobreescribible, nada remoto. Requisito 3 (escapado seguro): las escenas y bloques viajan como JSON dentro de un `<script type="application/json">`, que `guion.js` lee con `JSON.parse` y vuelca al DOM solo con `textContent` — nunca `innerHTML` ni interpolacion directa en el marcado; `_json_seguro_para_script` neutraliza ademas `<`, `>` y `&` con su escape Unicode tras `json.dumps(ensure_ascii=False)`, porque el analizador HTML decide donde termina un `<script>` por su contenido, no por su atributo `type` (un bloque con el texto literal `</script>` lo cerraria igual sin ese escapado). Verificado contra un guion hostil real (`</script><script>alert(1)</script>`, `<img onerror=...>`) abierto en Chromium headless via Playwright (instalado solo para esta comprobacion manual, no es dependencia del proyecto): se renderiza como texto literal, sin dialogos ni errores de consola — igual que el criterio de aceptacion del requisito 5 sobre el reproductor generado en limpio. Requisito 4 (validador de auto-contencion): ya existia desde T-00 (`verificar_salidas.buscar_recursos_externos`); esta sesion lo activa de verdad anadiendo la etapa `generar_reproductor_fixture`, que genera el reproductor sobre el primer guion de `fixtures/reales/` (a falta de `fixtures/guion-ejemplo.md`, que trae T-32) para que la etapa deje de ser NO APLICABLE, tal como ya anticipaba la decision de T-00 sobre la cuarta red. Alcance deliberadamente minimo: sin indice navegable, pantalla completa, resaltado ni autoscroll — eso es T-19 a T-22; este modulo solo demuestra que la canalizacion de un unico archivo autocontenido funciona de punta a punta. Suite: 262 pasan + 1 skipped (antes 252 + 1; 10 tests nuevos en `tests/test_reproductor.py`). Las cuatro verificaciones en verde, con "Guion de ejemplo" y "Generación de salidas" siguiendo NO APLICABLE hasta T-32 y T-30 respectivamente. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-19 (índice de escenas y entrada a pantalla completa), segunda de FASE B4.
>
> **Sesión anterior (2026-09-01) — T-17 (revalidación: releer, respetar y recalcular) COMPLETADA por una sesión de nube.** `scripts/revalidacion.py` nuevo: cierra el ciclo iterable que T-16 dejó preparado (validar → editar → revalidar). `revalidar_guion` relee `guion-escenas.md` con las mismas funciones de lectura de T-15/T-16 (`extraer_decisiones`, `extraer_texto_bloques`, `extraer_estado_revision`, sin ninguna nueva), funde las propuestas frescas con el historial de `estado.reescrituras` (T-15) y aplica las decisiones marcadas en el documento. Identidad estable entre pases: cada bloque de origen aporta su índice 0-based dentro de la escena (estable mientras el `.md` no cambie); una partición aceptada reparte ese índice en dos mitades (`'a'`/`'b'`), así que aceptar una partición en una pasada no confunde la numeración de los bloques de otras escenas ni pierde una edición manual ya registrada en otro bloque. Una edición manual se detecta comparando el texto del ancla contra lo que el sistema derivaría sin ella, y nunca se sobrescribe (invariante (c)); una decisión ya tomada (aceptar/rechazar) nunca vuelve a proponerse como pendiente (invariante (b), reutiliza `fusionar_con_estado`/`aplicar_decisiones` de T-15 tal cual). Informe de incidencias (requisito 3) solo con lo roto: bloques fuera de rango, escenas sin locución, marcas de reescritura con id desconocido, indicaciones que se colaron dentro de la locución, avisos de desviación de duración ya calculados por T-12 — nada de repetir lo que ya está bien. Refactor de apoyo en `tiempos.py` (T-12): `calcular_tiempos` se divide en `bloques_respiracion_marcados` (ahora pública) + el nuevo núcleo `calcular_tiempos_desde_marcados`, parametrizado por los bloques de una escena en vez de derivarlos siempre por reclasificación — comportamiento de `calcular_tiempos` sin cambios, verificado por la suite existente sin tocar un solo test de `test_tiempos.py`. `SEPARADOR_MITADES` de `reescrituras.py` pasa a público por el mismo motivo. Límite documentado (no un hallazgo, una decisión de alcance): una normalización aceptada sobre un bloque que ADEMÁS tiene una partición aceptada no se materializa en ninguna de las dos mitades (los offsets de la normalización se calcularon sobre el bloque entero); no se pierde nada — la decisión queda intacta en `estado.json`, invariante (b) — solo no se aplica en ese cruce concreto de dos reescrituras sobre el mismo bloque. Suite: 252 pasan + 1 skipped (antes 243 + 2; 8 tests nuevos en `tests/test_revalidacion.py`, incluido el ciclo de tres pasadas encadenadas del criterio de aceptación literal, y el test de idempotencia de `tests/test_logica_pendiente.py` que estaba en `skip` desde T-03 ya implementado de verdad, tal como pedía su motivo). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-18 (esqueleto del reproductor autocontenido), primera de FASE B4.
>
> **Sesión de PM 2026-09-01 (gestión de roadmap, sin código):** revisado el registro de hallazgos de `auditoriacontinua.md` — los tres ABIERTOS (#5, #6, #8) ya estaban enrutados a R-01, R-06 y T-32 respectivamente, ninguno nuevo que abrir. `FEEDBACK.md` sigue sin entradas `nuevo` (todavía no hay rodaje real). Añadida **R-07** a `ROADMAP_PRODUCTO.md` (oleada v3): capítulos de YouTube con marcas de tiempo reales, aprovechando la sección auxiliar `Capítulos` que T-08 ya detecta y las tomas reales de R-02, sin depender de ningún cambio de convención de marcado ni de nuevas dependencias. Sin bloqueos ni preguntas nuevas para el dueño.
