...
- **`tarjetas.json`** — contrato de intercambio con la generación del `.pptx` (T-29, `references/contrato-tarjetas.md`), pero reutilizable por cualquier consumidor: trae `duracion_estimada_segundos` por escena en el mismo orden que el guion, `duracion_real_segundos` (R-13, la misma duración de la toma buena que usa `guion-alineado.srt`, `null` si la escena no la tiene todavía) y, desde **R-16**, ya trae resuelto por escena el rango absoluto `inicio_segundos`/`fin_segundos` (coherente con `guion-alineado.srt` cuando hay parte de rodaje, con `guion.srt` si no) — la fase de montaje lee esas dos claves directamente, sin sumar nada por su cuenta (el `.srt` no lleva ninguna marca de escena en su propio texto). Fórmula de cómo se calculan, a título informativo, en `references/contrato-montaje.md`.
...
**Nunca el binario "pelado"** (`ruff`, `mypy`, `pytest` sin `python -m` por delante): un
contenedor de nube puede traer un segundo juego preinstalado, más nuevo que el pineado y
