...
**Última actualización:** 2026-09-01 — T-08 (parser de Markdown y detección del separador de escenas) COMPLETADA por una sesión de nube. `scripts/parser.py` nuevo: `dividir_en_bloques` trocea el `.md` en un `Bloque` por encabezado (de cualquier nivel, ignorando `#` dentro de vallas de código) con posición trazable (línea inicio/fin), de forma que concatenar el `contenido` de todos los bloques en orden reconstruye el guión línea a línea sin pérdida. `elegir_separador`/`parsear_guion` deciden el nivel/patrón (derivado de los `#` iniciales de `PATRON_ENCABEZADO_ESCENA`, no fijado a mano) y clasifican cada encabezado del nivel elegido en tres pasos: coincide con el patrón → escena; si no, lista negra (`secciones_auxiliares`) y rótulo `**LOCUCIÓN**` en el cuerpo se consultan a la vez — ninguna señal o solo lista negra → auxiliar, solo locución → escena, **las dos a la vez → conflicto real**, se levanta `DeteccionEscenasAmbiguaError` con alternativas y sus consecuencias (nº de escenas, duración media) en vez de decidir en silencio (requisito 6 de T-08); lo mismo si ningún encabezado del nivel esperado casa con el patrón (ambigüedad de nivel, con una alternativa por nivel candidato `#`/`##`/`###`). La respuesta de nivel se persiste en `estado.SeparadorEscena` (T-07); la de un conflicto, ajustando `configuracion_efectiva.secciones_auxiliares` — sin ampliar el esquema de `estado.json`. `extraer_metadatos` captura los pares `**Clave:** valor` de cabecera sin esquema fijo (los tres guiones reales usan claves distintas). Detalle y alternativas descartadas en `DECISIONES_TECNICAS.md`. Suite: 109 pasan + 7 skipped (antes 91 pasan + 8 skipped; +18 tests en `tests/test_parser.py` nuevo, -1 skip: el talón `test_parser_reconoce_toda_escena_del_guion_real` de `tests/test_logica_pendiente.py` queda implementado como su propio criterio de aceptación, no como nota aparte). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-09 (clasificador locución / no locución).
