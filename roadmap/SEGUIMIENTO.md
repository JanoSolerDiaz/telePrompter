...
**Última actualización:** 2026-09-01 — T-05 (monitorización de errores local) COMPLETADA por una sesión de nube. `scripts/monitorizacion.py` nuevo: `ejecutar_con_diagnostico(funcion, carpeta_salida)` captura cualquier excepción no controlada, vuelca tipo+mensaje+traceback (nunca variables locales, para no arrastrar el contenido del guión) a `<carpeta_salida>/diagnostico-<timestamp>.log`, deja constancia en el logger de T-02 y muestra al dueño solo un mensaje accionable genérico en español; termina en código de salida distinto de 0. `ResumenEjecucion` (dataclass) cubre el resumen final que pide el criterio de aceptación: escenas procesadas, bloques, avisos, reescrituras y salidas generadas. Como en T-02/T-04, es infraestructura sin punto de entrada real todavía — T-07 en adelante deben envolver su `main()` con `ejecutar_con_diagnostico` y rellenar `ResumenEjecucion`. Detalle en `DECISIONES_TECNICAS.md`. Suite: 44 pasan + 8 skipped (antes 35 pasan + 8 skipped; +9 tests nuevos en `tests/test_monitorizacion.py`, incluida una regresión que fija que el volcado no debe capturar variables locales). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-06 (robustez de entrada).
