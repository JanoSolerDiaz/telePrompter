...
> - **§3 Bloqueos** = tu lista de tareas. Quedan **dos**: instalar el paquete de `480-branded-pptx` (solo afecta a la salida `.pptx`) y probar el clicker Bluetooth real contra el mapa de teclas ya implementado (T-24). La funcionalidad asociada queda *latente* hasta que las resuelvas.
...
| T-24 | Atajos y clicker Bluetooth | **COMPLETADA** | 2026-09-02 | Mapa completo configurable (`mapa_teclas_reproductor`), antirrebote y ayuda `?`. Verificación del clicker físico sigue en bloqueo #5 de §3 |
...
| 5 | Probar los atajos con el clicker Bluetooth real | T-24 | El mapa completo ya está implementado (T-24, 2026-09-02): `Espacio`, `PageUp`/`PageDown`, flechas, `+`/`-`, `[`/`]`, `R`, `H`, `Esc` y `?`. Abrir el reproductor generado, pulsar cada botón del mando y decir qué tecla envía cada uno — si no coincide con el mapa vigente (visible con `?` dentro del reproductor), el agente lo ajusta en `Configuracion.mapa_teclas_reproductor`. | ABIERTO |
