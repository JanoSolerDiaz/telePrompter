...
**Última actualización:** 2026-09-01 — T-06 (robustez de entrada) COMPLETADA por una sesión de nube. `scripts/entrada.py` nuevo, delante del futuro parser (T-08): `validar_ruta_guion` (existe, no es carpeta, tamaño ≤ `TAMANO_GUION_MAX_BYTES`), `leer_guion` (UTF-8 con o sin BOM vía `utf-8-sig`, rechaza cualquier otra codificación y el contenido vacío/solo blancos), `verificar_estructura_minima` (cuenta encabezados de cualquier nivel — no solo el patrón estricto de escena, para que una entrada hostil no lo esquive — y rechaza 0 o más de `ESCENAS_MAX`), `nombre_guion_seguro`/`carpeta_salida_para` (derivan `<carpeta-del-guion>/<nombre-guion>-tarjetas/` saneando el nombre y verificando por `Path.relative_to` que nunca cae fuera de la carpeta del guión) y `ejecutar_con_limite_de_tiempo` (tope de tiempo con `ThreadPoolExecutor`, no con `signal.alarm`, que no existe en Windows). Toda la superficie usa una única excepción, `EntradaError`, con mensaje ya accionable en español. `config.py` gana `TIEMPO_PROCESO_MAX_SEGUNDOS` (60 s por defecto) junto a `TAMANO_GUION_MAX_BYTES`/`ESCENAS_MAX`, reservados desde T-00. Detalle y alternativas descartadas en `DECISIONES_TECNICAS.md`, incluida la nota sobre el `python` real de este contenedor (3.11.15 pese a `requires-python = ">=3.12"`). Suite: 64 pasan + 8 skipped (antes 44 pasan + 8 skipped; +20 tests nuevos en `tests/test_entrada.py`). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-07 (estado del proyecto de guión, `estado.json`).
