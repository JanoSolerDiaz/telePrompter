...
**Última actualización:** 2026-09-01 — T-07 (estado del proyecto de guión, `estado.json`) COMPLETADA por una sesión de nube. `scripts/estado.py` nuevo: `EstadoProyecto` fija el contrato de datos (versión de esquema, `InfoGuion` con ruta/hash sha256/tamaño del guión de origen, configuración efectiva, `SeparadorEscena` nivel/patrón para que T-08 los rellene, y los contenedores `escenas`/`reescrituras`/`validacion`/`salidas_generadas`, vacíos por ahora — infraestructura sin productor de datos todavía, mismo tratamiento que T-02/T-04/T-05). `guardar_estado` escribe de forma **atómica** (`estado.json.tmp` en la misma carpeta + `Path.replace()`, atómico en POSIX y Windows; si falla antes del reemplazo, limpia el temporal y el `estado.json` anterior queda intacto). `cargar_estado` lee, migra y reconstruye, con `EstadoError` (mensaje ya accionable, mismo contrato que `EntradaError` de T-06) ante archivo ausente, JSON corrupto o estructura incompleta. `guion_modificado`/`avisar_si_guion_modificado` detectan por hash sha256 si el guión cambió desde la última pasada y avisan qué se recalculará. Mecanismo de migraciones nuevo en `scripts/migraciones/` (`NNN_<nombre>.py`, cargadas con `importlib.import_module` por el prefijo numérico — mismo mecanismo que Django, `N999` de ruff ignorado ahí a propósito); la migración `001_estado_inicial.py` establece el esquema versión 1 preservando cualquier clave ya presente en los datos (invariante (c) de §0.2) y completando solo lo que falte. Detalle y alternativas descartadas en `DECISIONES_TECNICAS.md`. Suite: 91 pasan + 8 skipped (antes 64 pasan + 8 skipped; +27 tests en `tests/test_estado.py`, +8 en `tests/test_migraciones.py`, incluido uno que simula un corte a mitad de escritura y comprueba que el estado anterior sobrevive — cubre el criterio de aceptación literal de T-07). Las cuatro verificaciones en verde. Sin hallazgos ABIERTOS de severidad alta en `auditoriacontinua.md`. Siguiente tarea: T-08 (parser de Markdown y detección del separador de escenas).
